            ...
            <div className="fixed inset-0 z-[90] pointer-events-none overflow-hidden">
                ...
                >
                    ...
                    <div className="relative h-full flex flex-col justify-center pl-[20%] md:pl-[15%] pr-10 gap-8">
                        ...
                        {/*
                         * 修正ポイント:
                         * 元の実装では <style jsx> で .stroke-text を定義していたが、
                         * <Link> コンポーネントに対して jsx スコープのスタイルは
                         * 子要素のレンダリング後に適用されないケースがある。
                         *
                         * 対策: style 属性でインラインに -webkit-text-stroke を直接指定。
                         * hover は CSS で制御できないため、Tailwind の group + CSS変数で代替。
                         * → nav-stroke-text クラスを globals.css に定義して解決。
                         */}
                        {MENU_ITEMS.map((item, index) => (
                            <Link
                                ...
                                onClick={() => setIsOpen(false)}
                                className={`nav-stroke-text group relative block w-fit text-2xl md:text-5xl font-jura font-bold tracking-widest transition-all duration-500 transform ${isOpen ? "translate-x-0 opacity-100" : "translate-x-20 opacity-0"}`}
